...
class SparseCIFAR10Dataset(Dataset):
    ...
    def _create_sparse_mask(self) -> np.ndarray:
        ...
        print(
            f"Created sparse mask: {mask.sum()}/{len(mask)} samples labeled ({self.sparsity*100:.1f}%)"
        )
        return mask
...
def main():
    ...
    train_dataset = SparseCIFAR10Dataset(
        root="./data", train=True, sparsity=sparsity, transform=transform, download=True
    )

    test_dataset = SparseCIFAR10Dataset(
        root="./data",
        train=False,
        sparsity=1.0,  # Test set fully labeled
        transform=transform,
        download=False,
    )
